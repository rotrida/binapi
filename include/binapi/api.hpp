...
struct api {
    ...
    result<klines_t>
    klines(const std::string &symbol, const std::string &interval, std::optional<std::size_t> limit, std::optional<int64_t> start_time, std::optional<int64_t> end_time, klines_cb cb = {}) { return klines(symbol.c_str(), interval.c_str(), limit, start_time, end_time, std::move(cb)); }
    result<klines_t>
    klines(const char *symbol, const char *interval, std::optional<std::size_t> limit, std::optional<int64_t> start_time, std::optional<int64_t> end_time, klines_cb cb = {});
