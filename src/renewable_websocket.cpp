...
void renewable_websocket::switch_to_secondary_channel()
{
    ...
    _websocket.unsubscribe(_active_channel);
    _active_channel = _secondary_channel;
    _secondary_channel = nullptr;
}

void renewable_websocket::unsubscribe()
{
    const auto unsubscribe_channel = [&](auto& handle)
    {
        if (handle)
        {
            _websocket.async_unsubscribe(handle);
            handle = nullptr;
        }
    };
